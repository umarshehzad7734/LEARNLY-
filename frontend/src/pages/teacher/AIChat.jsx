...
import { Send, Bot, User, Loader, Plus, MessageSquare, PanelLeftClose, PanelLeft, MoreHorizontal, Moon, Sun, Search, Sparkles, Library, ChevronDown, Check, Zap, Cpu, X } from 'lucide-react'
...
const AIChat = () => {
  ...
  const startNewChat = () => {
    ...
    setMessages([
      {
        role: 'assistant',
        content: "Hello! I'm your Learnly AI Assistant. I can help you create lesson plans, generate quizzes, or analyze student performance. How can I help you today?"
      }
    ])
  }
  ...
  const chatHistory = [
    { id: 1, title: 'Lesson Plan: Biology 101' },
    { id: 2, title: 'Quiz Generation: Week 3' },
    { id: 3, title: 'Student Performance Analysis' },
  ]
      ...
      <AnimatePresence mode='wait'>
        {sidebarOpen && (
          ...
          >
            ...
            {/* History List */}
            <div className="flex-1 overflow-y-auto px-2 scrollbar-thin scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20">
              <div className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">History</div>
              {sessions.length === 0 ? (
                <div className="text-xs text-slate-600 px-4 mt-2 italic font-medium">No conversations yet</div>
              ) : (
                <div className="space-y-1">
                  {sessions.map(s => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-3 rounded-lg transition-colors text-left group ${currentSessionId === s.id ? 'bg-white/15 text-white' : 'hover:bg-white/10 text-slate-400'}`}
                    >
            ...
            <div className="p-4 border-t border-white/10">
              <div className="flex items-center gap-3 w-full px-3 py-3">
                <div className="w-8 h-8 rounded-full bg-[#14BF96] flex items-center justify-center text-white font-bold text-xs shadow-lg shadow-teal-500/20">
                  T
                </div>
                <div className="text-sm font-bold text-white uppercase tracking-tight">Teacher Console</div>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="flex-1 flex flex-col relative bg-white">
          ...
          <div className="flex items-center gap-2 overflow-hidden mr-4">
            ...
            <div className="min-w-0">
              <span className="font-bold text-slate-800 flex items-center gap-2 text-sm md:text-base">
                ...
                <span className="text-slate-300 text-[9px] font-black uppercase tracking-widest hidden lg:inline bg-slate-100 px-1.5 py-0.5 rounded">Teacher</span>
              </span>
            </div>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white to-transparent pt-10 pb-8 px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="text-center mt-3 text-xs text-slate-400 font-bold uppercase tracking-widest opacity-60">
              Learnly Assistant • Powered by AI
            </div>
          </div>
        </div>
      </div>
